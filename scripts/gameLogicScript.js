...
function getValidMoves(piece, diceValue, playerColor) {
    ...
    const finalMoves = validMoves.filter(move => {
        ...
        if (moveIsToEnemyTerritory) {
            if (playerColor === 'red') {
                return false; // Bloqueia linha 3 para vermelhas
            }
...
function canActivatePiece(piece, playerColor) {
    ...
    const finalMoves = validMoves.filter(move => {
        ...
        if (moveIsToEnemyTerritory) {
            if (playerColor === 'red') {
                return false; // Bloqueia durante ativação
            }
            if (piece.hasCompletedEnemyTerritory) {
                return false;
            }
...
async function handlePieceClick(cellIndex) {
    if (!gameState.gameActive || gameState.diceValue === 0) {
        updateMessage("Role os dados primeiro!");
        return;
    }

    // MODO ONLINE: Verifica se é a vez do jogador
    ...
    if (! piece.active) {
        if (gameState.diceValue === 1) {
            ...
            if (validMoves.length === 1) {
                ...
                // MODO ONLINE: Notifica servidor sobre ativação + movimento automático
                if (window.OnlineGame && window.OnlineGame.isOnlineMode()) {
                    const moveCell = getCellIndex(move.row, move.col, gameState.boardSize);
                    const state = window.OnlineGame.getOnlineState();
                    const result = await window.OnlineGame.notifyMove(state. myNick, state.myPassword, state.gameId, moveCell);

                    if (! result.success) {
                        alert(`Erro ao notificar jogada: ${result.error}`);
                        return;
                    }
                    // Movimento será processado via SSE
                    return;
                }

                // MODO LOCAL: Executa movimento normalmente
...
async function handleMoveClick(cellIndex) {
    ...
    // MODO ONLINE: Notifica servidor
    if (window.OnlineGame && window.OnlineGame.isOnlineMode()) {
        const state = window.OnlineGame. getOnlineState();
        const result = await window.OnlineGame.notifyMove(state.myNick, state.myPassword, state.gameId, cellIndex);
...
// ================================
// LISTENER PARA ATUALIZAÇÕES ONLINE
// ================================
window.addEventListener('onlineGameUpdate', (event) => {
    const data = event.detail;
    console.log('Processing online game update:', data);

    // TODO:  IMPLEMENTAR após receber especificação das respostas
    // Este handler processará todos os eventos recebidos via SSE

    // Estrutura esperada (AJUSTAR conforme servidor):
    // {
    //   type: 'start' | 'roll' | 'move' | 'turn' | 'win' | 'opponent_left',
    ...
    if (data.winner) {
        ...
        // Atualizar dados lançados
        handleOnlineRoll(data);
    } else if (data. move !== undefined) {
        // Executar movimento recebido
        handleOnlineMove(data);
    } else if (data.turn !== undefined) {
        // Trocar turno
        handleOnlineTurnChange(data);
    }
});

// Handlers auxiliares para eventos online (IMPLEMENTAR após especificação)
function handleOnlineRoll(data) {
    console.log('Online roll received:', data);
    // TODO: Processar resultado de dados
}

function handleOnlineMove(data) {
    console.log('Online move received:', data);
    // TODO: Processar movimento
}

function handleOnlineTurnChange(data) {
    console.log('Online turn change received:', data);
    // TODO: Atualizar turno
}
